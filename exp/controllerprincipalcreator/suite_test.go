...
func setup() {
	...
	if err := (&infrav1.AWSClusterControllerPrincipal{}).SetupWebhookWithManager(testEnv); err != nil {
		panic(fmt.Sprintf("Unable to setup AWSClusterControllerPrincipal webhook: %v", err))
	}

	err = (&AWSControllerPrincipalReconciler{
		Client: testEnv,
		Log:    log.Log,
	}).SetupWithManager(testEnv.Manager, controller.Options{})
	if err != nil {
		panic(fmt.Sprintf("Failed to add AWSControllerPrincipalReconciler to the envtest manager: %v", err))
	}
