...
func TestAWSStaticPrincipalTypeProvider(t *testing.T) {
	...
	staticProvider = NewAWSStaticPrincipalTypeProvider(&infrav1.AWSClusterStaticPrincipal{}, secret)
	...
	rolePrincipal2 := &infrav1.AWSClusterRolePrincipal{
		Spec: infrav1.AWSClusterRolePrincipalSpec{
			AWSRoleSpec: infrav1.AWSRoleSpec{
	...
	testCases := []struct {
		...
		},
		{
			...
			expect: func(m *mock_stsiface.MockSTSAPIMockRecorder) {
				m.AssumeRoleWithContext(gomock.Any(), &sts.AssumeRoleInput{
					RoleArn:         aws.String(rolePrincipal.Spec.RoleArn),
					RoleSessionName: aws.String(rolePrincipal.Spec.SessionName),
					DurationSeconds: pointer.Int64Ptr(int64(rolePrincipal.Spec.DurationSeconds)),
		...
		},
		{
			...
			expect: func(m *mock_stsiface.MockSTSAPIMockRecorder) {
				...
				m.AssumeRoleWithContext(gomock.Any(), &sts.AssumeRoleInput{
					RoleArn:         aws.String(rolePrincipal2.Spec.RoleArn),
					RoleSessionName: aws.String(rolePrincipal2.Spec.SessionName),
					DurationSeconds: pointer.Int64Ptr(int64(rolePrincipal2.Spec.DurationSeconds)),
				}).Return(&sts.AssumeRoleOutput{
					Credentials: &sts.Credentials{
		...
		},
		{
			...
			expect: func(m *mock_stsiface.MockSTSAPIMockRecorder) {
				...
				// AssumeRoleWithContext() call is not needed for rolePrincipal as it has unexpired credentials
				m.AssumeRoleWithContext(gomock.Any(), &sts.AssumeRoleInput{
					RoleArn:         aws.String(rolePrincipal.Spec.RoleArn),
					RoleSessionName: aws.String(rolePrincipal.Spec.SessionName),
					DurationSeconds: pointer.Int64Ptr(int64(rolePrincipal.Spec.DurationSeconds)),
